...
export default function DocumentUpload() {
  ...
  const deleteFromBackend = async (item: Entry) => {
    ...
    if (activeTab === "optimized") {
      const updatedOptimizedList = optimizedList.filter(opt => 
      ...
      const updatedCoverList = coverList.filter(cover => 
  ...
  const DocsTable = ({
    ...
    <div className="border rounded-lg overflow-hidden">
      <div className="grid grid-cols-12 bg-gray-100 text-sm font-bold px-4 py-3 ">
        {showDelete && <div className="col-span-1">Delete</div>}
        <div className={showDelete ? "col-span-4" : "col-span-5"}>Title</div>
      ...
      {items.length === 0 ? (
        ...
        <ul className="divide-y flex flex-col flex-col-reverse">
          {items.map((it, i) => (
            <li
              key={i}
              className="grid grid-cols-12 items-center px-4 py-4 hover:bg-gray-50"
            >
              {showDelete && (
              ...
              <div 
                className={showDelete ? "col-span-4 min-w-0" : "col-span-5 min-w-0"}
                onClick={() => onPick(it)}
