      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="bg-white/70 backdrop-blur-sm rounded-3xl shadow-xl border border-white/20 mb-8">
          <div className="border-b border-gray-200/50">
            <nav className="flex space-x-1 px-6">
              {[
                ...
                { key: 'spend', label: 'AI Spend', icon: TrendingUp }
              ].map(({ key, label, icon: Icon }) => (
                <button
                  key={key}
                  onClick={() => setActiveTab(key as 'overview' | 'users' | 'history' | 'sessions' | 'Admin' | 'resume' | 'recruiters' | 'activity' | 'spend')}
          ...
          <div className="p-8">
            ...
            {activeTab === 'spend' && (
              <SpendCharts apiBaseUrl={API_DASHBOARD} />
            )}
          </div>
        </div>
