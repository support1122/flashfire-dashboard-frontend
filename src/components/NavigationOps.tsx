...
// Operator-facing navigation: the horizontal top bar operators already know.
// Clients get the sidebar in NavigationClient.tsx instead. Navigation.tsx picks
// between the two by role. Kept deliberately close to the production original,
// so anything changed here should be a conscious decision rather than drift.
//
// NavigationProps is shared with NavigationClient. The document fields are
// accepted and ignored here: the top bar has no Documents sub-menu, so
// ResumeOptimizer1 falls back to its own internal tabs.
interface TabItem {
  id: string;
  label: string;
...
const NavigationOps: React.FC<NavigationProps> = ({
  activeTab,
  onTabChange,
